...
- **Palette Preview (`<an-palette-preview>`)**: A dedicated Web Component displaying 9 swatches (`c100` through `c900`). If `palette` is `null` (the default), it renders nothing (no DOM nodes). Displays hex value below each swatch.
...
1. **`<an-color-picker>` (`src/demo/component/color-picker/color-picker.ts`)**:
...
- **`src/demo/component/color-picker/color-picker.test.ts`**:
...
- **`src/demo/component/palette-preview/palette-preview.test.ts`**:
  ...
  - Verify the hex value is displayed below each swatch.
