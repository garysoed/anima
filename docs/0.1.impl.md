...
- [ ] **1. Phase 1: Build Tooling & SASS Pipeline**
  - [ ] **1.1 Dependencies & Package Scripts (`package.json`)**
    - [ ] 1.1.1 Add dependencies: `lit`, `@lit-labs/signals`, `sass`.
    - [ ] 1.1.2 Add Rollup devDependencies: `rollup`, `@rollup/plugin-typescript`, `@rollup/plugin-node-resolve`, `@rollup/plugin-commonjs`, `@rollup/plugin-terser`.
    - [ ] 1.1.3 Add scripts: `"build:demo": "rollup -c"`, `"start": "npx serve dist/demo"`.
  - [ ] **1.2 Rollup Configuration (`rollup.config.mjs`)**
    - [ ] 1.2.1 Configure `localPkgsResolver` for `gs-tools/export/*` resolution.
    - [ ] 1.2.2 Implement `litCssPlugin` to compile `.scss` strings using `sass.compileString` and export them wrapped in Lit's `css` template tag.
    - [ ] 1.2.3 Configure bundle targets for `src/demo/main.ts` outputting to `dist/demo/bundle.js`.
  - [ ] **1.3 TypeScript Compilation (`tsconfig.json`)**
    - [ ] 1.3.1 Ensure `tsconfig.json` supports experimental decorators or standard Lit decorator compilation and imports from `.scss`.
    - [ ] 1.3.2 Add type declarations for `.scss` module imports (`src/declarations.d.ts`).
  - [ ] **1.4 Documentation Updates (`README.md`, `src/README.md`)**
    - [ ] 1.4.1 Update root `README.md` to document new build scripts and `docs/` subdirectories.
    - [ ] 1.4.2 Update `src/README.md` to document the `demo/` subdirectory.
