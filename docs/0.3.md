...
- **Core Typography Module (`src/core/typography/`)**:
  - Introduce pure, DOM-independent typography data models and canonical constants:
    ...
    - `getTypographyCssFont(value: TypographyValue): string`: utility converting a `TypographyValue` into a CSS `font` shorthand string (`${fontWeight} ${fontSize}/${lineHeight} ${fontFamily}`).
...
graph TD
    subgraph Core Library ["src/core/"]
        subgraph Typography Module ["src/core/typography/"]
            TV["typography.ts (TypographyValue, TypographySet)"]
            TCF["getTypographyCssFont()"]
        end
...
1. **`src/core/typography/`**:
   - `typography.ts`: Canonical interfaces and constants for typography definitions. Zero DOM dependencies.
   - `getTypographyCssFont`: Pure function converting a `TypographyValue` to CSS `font` shorthand.
2. **`src/core/penpot/`**:
...
### 4.1 Typography Module Interfaces (`src/core/typography/typography.ts`)
...
export function getTypographyCssFont(value: TypographyValue): string;
```
...
1. **Unit Testing (`src/core/typography/typography.test.ts`)**:
   - Verify `getTypographyCssFont` generates correct CSS `font` shorthand strings.
