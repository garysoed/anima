...
- **Penpot Multi-Set Token Export Separation (`src/core/penpot/`)**:
  ...
  - Theme set contains the 8 color theme groups (`${seedName}-${mode}_${type}`) referencing palette swatches via `{${palette_type}.${shade}}`, `{white}`, and `{black}`.
...
graph TD
    ...
    subgraph Demo Application ["src/demo/"]
        ...
        Fonts["google-fonts.ts (Curated lists)"]
...
```json
{
  ...
  "theme": {
    "main-light_0": {
      "background": { "$type": "color", "$value": "{main_neutral.100}" },
      "primary": { "$type": "color", "$value": "{main_neutral.900}" },
      "secondary": { "$type": "color", "$value": "{main_neutral.700}" },
      "display": { "$type": "color", "$value": "{main_highlight.500}" },
...
The demo preloads Google Fonts with weights 300, 400, and 500 via `<link>` stylesheet elements in `index.html`.
...
2. **Unit Testing (`src/core/penpot/get-penpot-tokens.test.ts`)**:
   ...
   - Verify `theme` set contains all 8 theme groups with aliases `{...}`.
